...
function ZoneSection(props) {
    ...
    return <div className="zone-section" data-id={props.zone.id}>
        <h4>
            <span>
                <strong>{ cockpit.format(_("$0 Zone"), props.zone.name) }</strong>
                { props.zone.interfaces.length > 0 && <span>&nbsp;&nbsp;&nbsp;<strong>{_("Interfaces")}</strong> {props.zone.interfaces.join(", ")}</span> }
                { props.zone.source.length > 0 && <span>&nbsp;&nbsp;&nbsp;<strong>{_("Addresses")}</strong> {props.zone.source.join(", ")}</span> }
            </span>
            <span>{deleteButton}&nbsp;{addServiceAction}</span>
        </h4>
        ...
        <Listing columnTitles={[_("Service"), _("TCP"), _("UDP"), ""]}
            ...
            {props.zone.services.map(s => <ServiceRow key={firewall.services[s].id}
...
class AddServicesModal extends React.Component {
    ...
    render() {
        ...
        const titleText = cockpit.format(_("$0 to $1 zone"), addText, this.props.zoneName);
        return (
